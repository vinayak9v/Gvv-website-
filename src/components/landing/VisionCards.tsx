...
export default function VisionCards() {
  ...
  const handleCardClick = (e: React.MouseEvent, href: string, id: number) => {
    ...
    // 1. Jis card par click kiya hai uska "Black Hole" suck-in effect
    gsap.to(`.card-${id}`, {
      scale: 0, // Card shrink hoke zero ho jayega
      rotationZ: 1080, // 3 baar tezi se ghumega (360 * 3)
      opacity: 0,
      filter: 'blur(15px)', // Spaghettification (khinchne) ka illusion dega
      duration: 0.8,
      ease: 'expo.in', // Shuru mein slow, end mein ekdum fast (black hole gravity jaisa)
      transformOrigin: 'center center', // Center se khinchega
      onComplete: () => {
        router.push(href); // Pura andar jane ke baad naya page open hoga
      },
    });

    // 2. Baaki bache hue cards ko piche ki taraf fade karke gayab karna
  ...
  return (
    <section ref={ref} className="w-full flex justify-center items-center py-16 px-4">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-6xl w-full">
        {cardsData.map((card) => (
          <div key={card.id} className={`vision-card-wrap card-${card.id}`}>
            <div className="group flex flex-col items-center bg-[#0a1445]/80 backdrop-blur-sm border border-blue-500/40 rounded-2xl p-8 shadow-2xl transition-all duration-500 hover:-translate-y-2 hover:bg-gradient-to-b hover:from-[#1a1f6b] hover:via-[#111880] hover:to-[#070b2d] hover:shadow-[0_0_35px_rgba(59,130,246,0.45)] hover:border-[#1D129E] cursor-pointer">
              ...
              <button
                ...
                className="bg-gradient-to-b from-[#fae27c] to-[#d6ad44] text-blue-950 font-bold text-sm tracking-wide px-8 py-2.5 rounded-full shadow-[0_0_15px_rgba(250,226,124,0.3)] group-hover:shadow-[0_0_25px_rgba(250,226,124,0.6)] group-hover:scale-105 hover:brightness-110 transition-all duration-300"
              >
                Discover
              </button>
            </div>
